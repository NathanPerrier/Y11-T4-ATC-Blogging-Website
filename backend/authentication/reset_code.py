from datetime import datetime, timedelta
from backend.config import *
from backend.forms.__init__ import create_email, send_email 
from backend.authentication.user import User
from backend.db import *

class ResetCode(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    user = relationship('User', backref=db.backref('reset_code', lazy=True))
    
    reset_code = db.Column(db.String(6), nullable=False)
     
    expiration_time = db.Column(db.DateTime, nullable=False)
    creation_time = db.Column(db.DateTime, nullable=False)
    
    @classmethod
    def check_email(cls, email):
        """Check if the email is in the database."""
        return db.session.query(User).filter_by(email=email).first()
    
    @classmethod
    def get_by_user_id(cls, db, user_id):
        return db.session.query(cls).filter_by(user_id=user_id).first()
    
    @classmethod
    def delete_by_user_id(cls, db, user_id):
        code = cls.get_by_user_id(db, user_id)  
        if code:  
            db.session.delete(code) 
            db.session.commit() 

    @staticmethod
    def generate_code(length=6):
        """Generate a random numerical reset code of the given length."""
        return ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(length)) 

    @staticmethod
    def send_reset_code(email, code):
        """Send the reset code to the user's email."""
        first_name = ResetCode.check_email(email).first_name
        send_email('contact.webgenieai@gmail.com', email, create_email(first_name, email, 'Verification Code', code))

    @classmethod
    def store_reset_code(cls, email, code, expiration_duration=1800): # 30 mins
        """Store the reset code in the database with an expiration time."""
        user = cls.check_email(email)
        if user:
            expiration_time = datetime.utcnow() + timedelta(seconds=expiration_duration)
            reset_entry = ResetCode(user_id=user.id, reset_code=generate_password_hash(code), expiration_time=expiration_time, creation_time=datetime.utcnow())
            db.session.add(reset_entry)
            db.session.commit()

    @classmethod
    def create_and_send_reset_code(cls, email):
        """Generate, send, and store the reset code."""
        if cls.check_email(email):
            if cls.check_active_code(email):
                code = cls.generate_code()
                cls.send_reset_code(email, code)
                cls.store_reset_code(email, code) #error here
                return True, None
            return True, None
        return False, 'Invalid Email'

        
    @classmethod
    def check_active_code(cls, email):
        if db.session.query(cls).filter_by(user_id=User.get_by_email(db, email).id).filter(ResetCode.expiration_time > datetime.utcnow()).first():
            return False
        return True
    
    @classmethod
    def check_code(cls, user_code, code):
        return check_password_hash(user_code, code)
    
    @classmethod    
    def check_reset_code(cls, email, input_code):
        reset_entry = db.session.query(cls).filter_by(user_id=User.get_by_email(db, email).id).order_by(ResetCode.creation_time.desc()).first()
    
        if not reset_entry:
            return False, "No reset code found for the user"

        if cls.check_code(reset_entry.reset_code, input_code) == False:
            print(reset_entry.reset_code, input_code)
            return False, "Invalid reset code"

        current_time = datetime.utcnow()
        if current_time > reset_entry.expiration_time:
            return False, "Reset code has expired"

        return True, None
    